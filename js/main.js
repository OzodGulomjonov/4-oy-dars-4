...
let normolizeMovies = moviesArray.map(item => {
    ...
    newObject.summary = item.summary;
...
function renderMovies(array,wrapper) {
    ...
    array.forEach(item => {
        ...
        templateItem.querySelector(".more__info-btn").dataset.movieid = item.id
...
elForm.addEventListener("submit" , function (evt) {
    ...
    if(selectSortValue == "A - Z") {
        filteredArray.sort((a,b)  => {
            return a.title - b.title
        })
    }

    if(selectSortValue == "Z - A") {
        filteredArray.sort((a,b)  => {
...
movieWrapper.addEventListener ("click" , (event) => {

    let currentMovieId = event.target.dataset.movieid;

    if(currentMovieId) {

        let founded = newArray.find(function (item) {
            return item.id == currentMovieId
        })
        
        modalTitle.textContent = founded.title;
        modalBody.textContent = founded.summary;
    }
})